...
plotbarber<-function(param,m,filename="none"){
   ...
   par(mfcol=c(2,3),ps=16,lwd=2 ) #one graph with large text
   ...
   mv=sweep(dc,MARGIN=1,(const$b+1)*const$vol,`*`) #mulitply concentration with volume and buffer to get amounts
   dcs=100*colSums(mv)
...
plotSensitivityBarber<-function(data,filename="none",exclude=NULL,include=NULL,maintitle="Sensitivity Analysis Barber-Cushman Model"){
   ...
   oldpar=par(no.readonly=TRUE)
   par(mfcol=c(1,1),ps=16,lwd=4 ) #one graph with large text
   ymax=0  
   for(p in 2:kn){
